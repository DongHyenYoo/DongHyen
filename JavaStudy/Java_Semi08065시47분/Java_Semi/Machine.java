...
/*
	...
	public static void rankingRC()throws IOException //1.1.2 제일 많이 팔린메뉴
	{
		...
		try
		{
			...
			{//한 문자열 줄로 가져올거고 String[가져온수][4] 이렇게 담길것
				for(int j=0;j<line.split(",").length;j++)                //line.split(",").length 이렇게써도 된다. 잘기억하기
				{
					if(j==0)
					{
					data += line;
			...
			//System.out.printf("%6s",data); 데이터 출력
			//System.out.println(row+"     "+column); 

			String[][] aSD = new String[row][column/row];  //aSD = all Sell Data
		                                               //[가져온 데이터의 행의 갯수][가져온 데이터의 열의 갯수]배열 선언
		...
		//	System.out.println(Arrays.asList(aSD[0][aSD[0].length]).get(0));
			...
			for(int i=0; i<aSD.length;i++)
			{
				for(int j=1;j<aSD.length;j++)
				{
					String restName="";
					String menu="";
					System.out.print(Arrays.asList(aSD[i][aSD[i].length-1]).indexOf("다운타우너"));
					menu = Arrays.asList(aSD[i][aSD[i].length-1]).get(1);

					//Arrays.asList(aSD[j][aSD[j].length]);
		...
		*/
		...
		{
			e.printStackTrace();
			System.out.println("판매된 데이터가 없습니다.이전메뉴로 돌아갑니다");
			rcMenu();
		}
