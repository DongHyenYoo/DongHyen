...
--2. 형식 및 구조
--①컬럼 레벨의 형식
-- 컬럼명 데이터 타입 [CONSTRAINT CONSTRAINT 명] NOT NULL
...
--※NOT NULL 제약조건은 컬럼 레벨로 명시하는 것을 추천한다.
...
/*
HR	SYS_C007199	TBL_TEST11	C	COL2	"COL2" IS NOT NULL --CHECK 를 쓰지않았는데 CHECK제약 조건이라고 나와있음	
HR	SYS_C007200	TBL_TEST11	P	COL1		
*/
...
-- 여기에 더하여, MODIFY 절을 통해 NOT NULL 제약조건을 추가한 테이블인
DESC TBL_TEST13;
/*
이름   널?       유형           
---- -------- ------------ 
...
/*
HR	SYS_C007199	TBL_TEST11	C	COL2	"COL2" IS NOT NULL	
HR	SYS_C007200	TBL_TEST11	P	COL1		
HR	TEST12_COL2_NN	TBL_TEST12	C	COL2	COL2 IS NOT NULL	
HR	TEST12_COL1_PK	TBL_TEST12	P	COL1		
HR	TEST13_COL1_PK	TBL_TEST13	P	COL1		
HR	TEST13_COL2_NN	TBL_TEST13	C	COL2	COL2 IS NOT NULL	
HR	SYS_C007205	TBL_TEST13	C	COL2	"COL2" IS NOT NULL	
*/
...
--2. 형식 및 구조
-- 컬럼명 데이터 타입 DAFAULT 기본값

--3. INSERT 명령시 해당 컬럼에 입력될 값을 할당하지 않거나,
-- DEFAULT 키워드를 활용하여 기본으로 설정된 값을 입력하도록 할 수 있다.

--4. DEFAULT 키워드와 다른 제약(NOT NULL등) 표기가 함께 사용되어야 하는 경우
-- DEFAULT 키워드를 먼저 표기(작성) 할것을 권장한다.

--○DEFAULT 표현식 실습
--테이블 생성
CREATE TABLE TBL_BBS      --게시판 테이블생성
( SID NUMBER         PRIMARY KEY  -- 게시물 번호 -> 식별자 -> 자동증가 
, NAME VARCHAR2(20)     -- 게시물 작성자
, CONTENTS VARCHAR2(200) --게시물 내용
, WRITEDAY DATE        DEFAULT SYSDATE NOT NULL --게시물 작성일
, COUNTS NUMBER        DEFAULT 0 --게시물 조회수
, COMMENTS NUMBER      DEFAULT 0 --게시물 댓글 개수
);

--SID를 자동 증가 값으로 운영하려면 시퀀스 객체가 필요
...
INSERT INTO TBL_BBS(SID,NAME,CONTENTS,WRITEDAY,COUNTS,COMMENTS)
VALUES(SEQ_BBS.NEXTVAL, '최동현' , '계속 실습중입니다.'
, SYSDATE ,0,0);

INSERT INTO TBL_BBS(SID,NAME,CONTENTS,WRITEDAY,COUNTS,COMMENTS)
VALUES(SEQ_BBS.NEXTVAL, '조현하' , '열심히 실습중입니다.'
, SYSDATE ,0,0);

INSERT INTO TBL_BBS(SID,NAME,CONTENTS,WRITEDAY,COUNTS,COMMENTS)
VALUES(SEQ_BBS.NEXTVAL, '한은영' , '엄청 실습중입니다.'
...
TBL_BBS	WRITEDAY	DATE			7			N	4	8	SYSDATE 											NO	NO		0		NO	YES	NONE
TBL_BBS	COUNTS	NUMBER			22			Y	5	24	"0 --게시물 조회수
"											NO	NO		0		NO	YES	NONE
TBL_BBS	COMMENTS	NUMBER			22			Y	6	28	"0 --게시물 댓글 개수
"											NO	NO		0		NO	YES	NONE
*/
