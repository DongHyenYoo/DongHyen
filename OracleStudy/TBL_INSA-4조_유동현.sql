...
/*SELECT NAME, CITY, BASICPAY, SSN
FROM TBL_INSA
WHERE CITY = '인천' AND BASICPAY BETWEEN 1500000 AND 2000000;
*/
...
SELECT NAME "이름", SSN "주민번호"
FROM TBL_INSA
WHERE TRUNC(SUBSTR(SSN,1,2),-1) = 70;
*/
...
SELECT NAME "이름"
FROM TBL_INSA
WHERE TRUNC(EXTRACT(YEAR FROM IBSADATE),-2) = 2000;
*/
...
SELECT EXTRACT(YEAR FROM SYSDATE) 
        - (TO_NUMBER(SUBSTR(SSN,1,2))+1899) "나이"
FROM TBL_INSA;
*/
...
WHERE TRUNC(EXTRACT(YEAR FROM SYSDATE) 
        - (TO_NUMBER(SUBSTR(SSN,1,2))+1899),-1) = 20;
*/
...
/*
SELECT NAME
        , EXTRACT(YEAR FROM SYSDATE) - (1899+TO_NUMBER(SUBSTR(SSN,1,2))) "나이" 
FROM TBL_INSA
...
SELECT NVL2(COMMISSION_PCT, '받는사람', '안받는사람') "구분"
       , COUNT(*) "인원수"
FROM EMPLOYEES
GROUP BY NVL2(COMMISSION_PCT, '받는사람', '안받는사람');
...
SELECT COUNT(*)
FROM TBL_INSA
GROUP BY BASICPAY+SUDANG<1000000;
...
SELECT BUSEO , DECODE(BUSEO,'개발부',COUNT(*),'총무부',COUNT(*), '영업부',COUNT(*))
FROM TBL_INSA
